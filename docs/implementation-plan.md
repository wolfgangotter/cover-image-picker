...
| F6a | `TextFileView.requestSave` is **debounced by two seconds**, while `processFrontMatter` works on the file on disk. | `obsidian.d.ts` TextFileView | Writing frontmatter while an editor holds unsaved changes is a race the user loses: the editor's later save lands with its stale copy and the property vanishes. `frontmatter-port.ts` flushes open editors for the note with `view.save()` first. |
